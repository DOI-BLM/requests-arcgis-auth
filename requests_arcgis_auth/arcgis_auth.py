...
class ArcGISPortalSAMLAuth(AuthBase):
    ...
    def __call__(self, prepared_request):
        ...
        prepared_request.register_hook('response', self._handle_expired_token)
    ...
    def _handle_expired_token(self, resp, **kwargs):
        # Handling Expired Tokens!!!  {u'error': {u'code': 498, u'details': [], u'message': u'Invalid token.'}}
        # type(r) = Response
    ...
    def _add_token_to_request(self, prepared_request):
        ...
        prepared_request.prepare_url(prepared_request.url, params = params)
        return prepared_request



### NEED TO DEAL WITH DUPLICATE TOKEN and F parameter keys in the requests still............

a=ArcGISPortalSAMLAuth(r'oeBg9SpWt0NPByQb',expiration=1)
a.capture_request_history=True
s=requests.session()
s.auth=a


r=s.get("https://blm-egis.maps.arcgis.com/sharing/rest/portals/self?f=json")
print r.request.url
print r.json().get("user").get("username")
print a._token_acquired
print a._token_data
